...
class GameGrid {
    var width: Int = 0
    var height: Int = 0
    private var grid: Array<Array<GameSymbol?>>

    constructor(width: Int, height: Int) {
        this.width = width
        this.height = height
        grid = Array(width) { arrayOfNulls<GameSymbol?>(height) }
    ...
    constructor(width: Int, height: Int, grid: Array<Array<GameSymbol?>>) {
        this.width = width
        this.height = height
        this.grid = grid
    }
    ...
    private fun copy(): GameGrid {
        val grid: Array<Array<GameSymbol?>> = Array(width) {
            arrayOfNulls(
                height
            )
        }
